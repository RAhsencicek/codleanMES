...
KAFKA_CONFIG = make_config(
    group_id="historical-data-pull-" + datetime.now().strftime("%H%M%S"),
    auto_offset_reset="earliest",  # ⭐ BURASI ÖNEMLİ: En baştan başla
    ...
    try:
        while True:
            ...
            if message_count % 1000 == 0:
                print(
                    f"   İşlenen: {message_count:,} | Bulunan: {found_in_range}",
                    end="\r",
                )
                                ...
                                if machine_id.startswith("HPR"):
                                    ...
                                    for sample in comp.get("samples", []):
                                        ...
                                        if sensor_id not in violations[machine_id]:
                                            violations[machine_id][sensor_id] = {
                                                "violations": []
                                            }
